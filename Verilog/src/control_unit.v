// Control Unit ??? Mikroprogramlanm???? Kontrol Birimi (??st Mod??l)
// CAR, SBR, Control Memory ve Microinstruction Decoder'?? birle??tirir.
module control_unit (
    input  wire        clk,
    input  wire        reset,
    ...
    // ?????? Ko??ul de??erlendirmesi ???????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????
    always @(*) begin
        case (cd)
            2'b00: condition = 1'b1;       // Ko??ulsuz
            2'b01: condition = ir_reg[15]; // i: dolayl?? bit
            2'b10: condition = ac_sign;    // S: AC i??aret biti
            2'b11: condition = ac_zero;    // Z: AC = 0
            default: condition = 1'b0;
        endcase
    end

    // ?????? CAR / SBR dallanma mant?????? ????????????????????????????????????????????????????????????????????????????????????????????????????????????
    always @(*) begin
        car_load      = 1'b0;
        car_inc       = 1'b1;
        car_load_data = 7'b0;
